...
server <- function(input, output, session) {
  ...
  Size <- 8  # Taille grille a changer
  mainGenerate()  # Initialiser avec une grille valide
  observe({
          ...
          observeEvent(input[[button_id]], {
            isolate({
              ...
              updateActionButton(session, paste0("cell_", row, "_", col), 
                                 label = as.character(GetCaseValue(row - 1, col - 1)))})
          ...
          lapply(1:Size, function(j) {
            ...
            color <- ifelse(value == 1, "lightblue", 
                            ifelse(value == 0, "lightgreen", 
                                   ifelse(value == 7, "white", "white")))  # Default color "white"
            actionButton(
